...
def p_array_expression(p):
    '''array_expression : ID '(' index_expression_list ')' 
                        ...
                        '''
    if p[2] == '(' and p[4] == ')':
        p[0] = c.ArrayExp(c.Variable(p[1]), p[3])
    elif p[2] == '[' and p[4] == ']':
        p[0] = c.ArrayExp(c.Variable(p[1]), p[3])
...
def p_init_expression(p):
    ''' init_expression     : ID "=" additive_expression 
                            | ID "=" ZEROS '(' additive_expression ')'
                            | ID "=" ONES '(' additive_expression ')'
                            | ID "=" EYE '(' additive_expression ')'  '''
    if len(p) == 4:
        p[0] = c.InitExpression(c.Id(p[1]), "=", p[3])
    elif p[3] == "zeros":
        print("even here")
        p[0] = c.InitExpression(c.Id(p[1]), "=", c.Zeros(p[5])) 
    elif p[3] == "ones":
        p[0] = c.InitExpression(c.Id(p[1]), "=", c.Ones(p[5]))
    elif p[3] == "eye":
        p[0] = c.InitExpression(c.Id(p[1]), "=", c.Eye(p[5]))
